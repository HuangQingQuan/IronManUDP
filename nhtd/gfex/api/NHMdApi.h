...
public:
	...
	****************************************/
	...
	/***************************************
	* \fn    OnRspAuthUserPassword
	* \brief 用户验证数据请求时响应
	* \param RspAuthUserPasswordField_t& rsp
	* \param TSequenceIDType nRequestID 
	****************************************/
	virtual void OnRspAuthUserPassword(const RspAuthUserPasswordField_t& rsp,TSequenceIDType nRequestID){};
	...
	****************************************/
	...
	/***************************************
	* \fn    OnRspQryHisMarketData
	* \brief 查询历史行情请求时响应
	* \param RspQryHisMarketDataField_t & rsp
	* \param TSequenceIDType nRequestID
	* \return void
	****************************************/
	virtual void OnRspQryHisMarketData(const RspQryHisMarketDataField_t& rsp,TSequenceIDType nRequestID){};
};
...
public:
	...
	****************************************/
	...
	/***************************************
	/* \brief 设置心跳超时时间。
	/* \param timeout 心跳超时时间(秒) ,超时时间最大为60秒，超过60秒系统按照60秒来处理 
	****************************************/
	...
	****************************************/
	...
	/***************************************
	* \fn    ReqAuthUserPassword(该方法暂时未用)
	* \brief 发送用户验证数据请求
	* \param ReqAuthUserPasswordField_t & req
	* \param TSequenceIDType nRequestID 请求序号
	* \return 0:成功，其他失败,返回成功只表明请求发送成功
	*			 处理结果在对应回调函数中检查
	****************************************/
	virtual int ReqAuthUserPassword(ReqAuthUserPasswordField_t& req,TSequenceIDType nRequestID) = 0;
	...
	*			 处理结果在对应回调函数中检查
	****************************************/
	virtual int ReqQryExchange(ReqQryExchangeField_t& req,TSequenceIDType nRequestID) = 0;
	...
	*			 处理结果在对应回调函数中检查
	****************************************/
	virtual int ReqQryInstrument(ReqQryInstrumentField_t& req,TSequenceIDType nRequestID) = 0;
	
	/***************************************
	* \fn    ReqQryHisMarketData
	* \brief 发送查询历史行情请求
	* \param ReqQryHisMarketDataField_t & req
	* \param TSequenceIDType nRequestID 请求序号
	* \return 0:成功，其他失败,返回成功只表明请求发送成功
	*			 处理结果在对应回调函数中检查
	****************************************/
	virtual int ReqQryHisMarketData(ReqQryHisMarketDataField_t& req,TSequenceIDType nRequestID) = 0;
protected:
	~CNhMdApi();
};
