...
//用户验证数据请求结构
typedef struct ReqAuthUserPasswordField
{
...
}ReqAuthUserPasswordField_t;

//用户验证数据应答结构
typedef struct RspAuthUserPasswordField 
{
	TRspCodeType response_code;		//响应代码, 0为成功，负数为各类错误
	TRspStringType response_string;	//响应文本信息
}RspAuthUserPasswordField_t;
...
//查询历史行情请求结构
typedef struct ReqQryHisMarketDataField 
{
	TIntType speedtype;				//速度类型:1原始速度,2
	TExchangeNoType exchange_id;			//交易所代码
	TContractNoType instrument_id;		//合约代码
	TDateType2 beg_date;				//开始日期
	TTimeType beg_time;				//开始时间
	TDateType2 end_date;				//结束日期
	TTimeType end_time;				//结束时间
}ReqQryHisMarketDataField_t;

//查询历史行情应答结构
typedef struct RspQryHisMarketDataField 
{
	TRspCodeType response_code;		//响应代码, 0为成功，负数为各类错误
	TRspStringType response_string;	//响应文本信息
	TIntType total_num;							//总共查询到的记录数
}RspQryHisMarketDataField_t;
